...
# Use this part in case of some additional datasets:
...
#	write out .log file for SNPs removed during LiftOver:
write.table(NULL, '../log_files/hg18_LiftOver_extra.log', sep = '\t', quote = F, row.names = F)
...
for (i in list.files()[str_detect(list.files(), '.*hg18.txt')]){
  ...
  #	change the path!!!
  write.table(output, paste('/groups/umcg-wijmenga/tmp04/umcg-uvosa/curated_GWAS_full_summary_statistics/filtered_SS_files/hg19_extra/', str_replace(i, 'hg18', 'hg18_hg19'), sep = ''), sep = '\t', quote = F, row.names = F)
  
  nr_removed <- data.frame(file = i, initiaNrSNPs = nrow(dat), removedLiftOver = nrow(dat) - nrow(output))
  write.table(nr_removed, file = '../log_files/hg18_LiftOver_extra.log', sep = '\t', quote = F, row.names = F, append = T, col.names=!file.exists("../log_files/hg18_LiftOver_extra.log"))
